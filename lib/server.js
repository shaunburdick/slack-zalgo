...
class Server {
   ...
   *
   * @constructor
   * @param {string} validationKey The validation key to use, can be multiple with comma
   */
  constructor(validationKey) {
    ...
    this.validationKey = validationKey.split(/\,\s+/);
  ...
  configureRoutes() {
    ...
    this.app.post('/zalgo', (req, res) => {
      if (
        req.body && req.body.token && this.validationKey.indexOf(req.body.token) !== -1
